...
- CIGAR 中 `I/S`（插入/软剪切）按 Bismark 逻辑先填充为 `X`，最终在 XM 上表现为 `.`。
- reverse 比对时按 Bismark 输出流程处理：先在工作方向计算 methcall，再在输出阶段反转 XM。
...
- SE 使用 `XR/XG` 组合判断输出链向；PE 使用 `YS+XR` 优先判断（缺失 `YS` 时回退到 SE 规则）。
